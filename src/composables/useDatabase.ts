...
export const useDatabase = () => {
  ...
  const addRecord = async (storeName: string, data: any): Promise<{ success: boolean; error?: string }> => {
    if (!db.value) {
  ...
  const getRecords = async (storeName: string, indexName?: string, key?: any): Promise<any[]> => {
    if (!db.value) return []
